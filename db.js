...
/* ---------- CONTA ADMIN (pedido + aprovação pelo dono) ---------- */

// Envia um pedido para virar administrador (fica pendente até o
// dono do site aprovar ou recusar).
async function dbRequestAdminAccess(teamName, message){
  const { data, error } = await sb.rpc("request_admin_access", {
...
// Pega o pedido mais recente do usuário logado (ou null se nunca pediu)
async function dbGetMyAdminRequest(userId){
  const { data, error } = await sb
    .from("admin_requests")
    .select("*")
    .eq("user_id", userId)
...
// Lista todos os pedidos pendentes (só o dono do site consegue ler isso — RLS)
async function dbGetPendingAdminRequests(){
  const { data, error } = await sb
    .from("admin_requests")
    .select("*")
    .eq("status", "pending")
...
async function dbApproveAdminRequest(requestId){
  const { data, error } = await sb.rpc("approve_admin_request", { request_id: requestId });
  if(error) throw error;
  return data;
}
...
/* ---------- PÁGINA DA EQUIPE (teams) ---------- */

async function dbGetMyTeam(adminId){
  const { data, error } = await sb
    .from("teams")
    .select("*")
    .eq("admin_id", adminId)
...
async function dbGetTeamById(id){
