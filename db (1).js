...
// Troca um código de convite por privilégio de administrador
// (a validação de verdade acontece dentro da função no banco).
async function dbRedeemAdminCode(code){
  const { data, error } = await sb.rpc("redeem_admin_code", { invite_code: code });
  if(error) throw error;
  return data;
}
